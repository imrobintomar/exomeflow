...
```bash
docker pull itsrobintomar/exomeflow:2.2.7
...
docker run --rm -it \
  ...
  itsrobintomar/exomeflow:2.2.7 run \
...
singularity pull exomeflow-2.2.7.sif docker://itsrobintomar/exomeflow:2.2.7

# Or build from the definition file
singularity build exomeflow-2.2.7.sif exomeflow.def
...
singularity exec \
  ...
  exomeflow-2.2.7.sif exomeflow run \
    --input-dir    /data/fastq \
    --output       /data/results \
    --reference    /refs/Homo_sapiens_assembly38.fasta \
    --dbsnp        /vcf/Homo_sapiens_assembly38.dbsnp138.vcf.gz \
